...
$deposits = $pdo->query("
    SELECT d.id, d.amount, d.method, d.status, d.request_date, u.email 
    FROM deposits d 
...
// En tête du fichier après la récupération des données
        ...
        <div class="row">
                    ...
                    <ul class="nav flex-column">
                        ...
                        <li class="nav-item">
                            <a class="nav-link <?= $action === 'withdrawals' ? 'active' : '' ?>" href="?action=withdrawals">
                                <i class="fas fa-money-check-alt me-2"></i> Retraits
                            </a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link <?= $action === 'deposits' ? 'active' : '' ?>" href="?action=deposits">
                                <i class="fas fa-coins me-2"></i> Dépôts
                            </a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link <?= $action === 'bets' ? 'active' : '' ?>" href="?action=bets">
                                 <i class="fas fa-coins me-2"></i> Paris
    </a>
                        </li>
                        <li class="nav-item">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                ...
                <?php elseif($action === 'matches'): ?>
                    ...
                    <?php elseif($action === 'withdrawals'): ?>
                    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                        <h1 class="h2">Demandes de retrait</h1>
                    </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td><?= htmlspecialchars($withdrawal['email']) ?></td>
                                    <td><?= isset($withdrawal['amount']) ? number_format($withdrawal['amount'], 2) : '0.00' ?> €</td>
                                    <td><?= ucfirst($withdrawal['method']) ?></td>
                                    <td><?= $withdrawal['request_date'] ?></td>
                                    <td>
                                        <?php if($withdrawal['status'] === 'pending'): ?>
                                            <a href="process_withdrawal.php?id=<?= $withdrawal['id'] ?>&action=approve" 
                                               class="btn btn-sm btn-success">
                                                <i class="fas fa-check"></i>
                                            </a>
                                            <a href="process_withdrawal.php?id=<?= $withdrawal['id'] ?>&action=reject" 
                                               class="btn btn-sm btn-danger">
                        ...
                        <table class="table table-striped">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td><?= isset($deposit['amount']) ? $deposit['amount'] : 'N/A' ?> €</td>
                ...
                <?php elseif($action === 'bets'): ?>
                    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                        <h1 class="h2">Gestion des paris</h1>
                    </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped">
                            ...
                            <tbody>
                                <?php 
                                $bets = $pdo->query("
                                    SELECT b.*, u.email, m.team1, m.team2 
                                    FROM bets b
                                ...
                                foreach($bets as $bet): ?>
                                <tr>
                                    <td><?= htmlspecialchars($bet['email']) ?></td>
                                    <td><?= htmlspecialchars($bet['team1']) ?> vs <?= htmlspecialchars($bet['team2']) ?></td>
                                    <td><?= $bet['bet_amount'] ?> €</td>
                                    <td><?= $bet['potential_gain'] ?> €</td>
                                    <td><?= $bet['status'] ?></td>
                                   
                                        <td>
    <a href="process_bet.php?id=<?= $bet['id'] ?>&result=win&csrf_token=<?= $_SESSION['csrf_token'] ?>" 
       class="btn btn-sm btn-success"
       onclick="return confirm('Confirmer la victoire ?')">
        <i class="fas fa-check"></i> Valider
    </a>
    <a href="process_bet.php?id=<?= $bet['id'] ?>&result=lose&csrf_token=<?= $_SESSION['csrf_token'] ?>" 
       class="btn btn-sm btn-danger"
       onclick="return confirm('Confirmer la défaite ?')">
    ...
    <script>
        ...
        if(editMatchModal) {
            editMatchModal.addEventListener('show.bs.modal', function(event) {
                ...
                const eventDate = matchData.event_date.replace(' ', 'T').substr(0, 16);
